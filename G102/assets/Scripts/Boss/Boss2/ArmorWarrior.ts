...
export default class ArmorWarrior extends Boss {
    ...
    private isEffCom(): boolean {
        ...
        if (GameEffectsManager.getInstance().map_node_pools.has(GameEffectId.xuanyun) &&
            GameEffectsManager.getInstance().map_node_pools.has(GameEffectId.boss2_normal_att) &&
            GameEffectsManager.getInstance().map_node_pools.has(GameEffectId.boss2_normal_att_hit1) &&
            GameEffectsManager.getInstance().map_node_pools.has(GameEffectId.boss2_normal_skill) &&
            GameEffectsManager.getInstance().map_node_pools.has(GameEffectId.boss2_normal_skill_hit)) {
            this.effcomp = true;
        }
        return this.effcomp;
    }
    ...
    checkSkill(dt: number) {
        this.skill_jishu += dt;
        if (!this.isHaveDeBuff(BuffId.Hero_XuanYun)&&this.isEffCom()) {
